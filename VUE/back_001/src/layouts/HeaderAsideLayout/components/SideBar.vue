<template>
    <scroll-bar>
        <div class="logo">
            ...
            <span class="site-name" style="font-size: 14px">基础服务平台</span>
        </div>
        <el-menu
                mode="vertical"
                :show-timeout="200"
            ...
            <template v-for="item in asideMenuConfig">
                ...
                <el-submenu v-else :index="item.name || item.path" :key="item.name">
                    ...
                    <template v-for="child in item.children" v-if="!child.hidden">
                        <a v-if="child.outlink" :href="child.path" :target="child.target || '_self'">
                            <el-menu-item :index="item.path + child.path">
                                <span v-if="child && child.name" slot="title">{{child.name}}</span>
                            </el-menu-item>
                        </a>
                        <router-link v-else :to="item.path + child.path" :key="child.name" :target="child.target || '_self'">
                            <el-menu-item :index="item.path + child.path">
                                <span v-if="child && child.name" slot="title">{{child.name}}</span>
                            </el-menu-item>
                        </router-link>
...
<script>
    ...
    export default {
        ...
        created () {
            this.activeName = this.$route.meta.active?this.$route.meta.active:this.$route.path;
        },
        motheds: {
